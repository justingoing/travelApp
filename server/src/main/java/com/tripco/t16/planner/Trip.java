...
public class Trip {
  ...
  private final double maxLat = 37; // note these are inverted since SVG goes top to bottom
  private final double minLat = 41; // we want out normalized plane to go top to bottom
  private final double maxLong = -102.05;
  private final double minLong = -109.05;

  private final double latDiff = this.maxLat - this.minLat;
  private final double longDiff = this.maxLong - this.minLong;
  ...
  {
    return (lat - this.minLat) / (this.latDiff);
  }
  ...
  {
    return (lon - this.minLong) / (this.longDiff);
  }
